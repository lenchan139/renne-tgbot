...
import { downloadTorrent, crawlMagnet, crawlTorrentBuffer, categorizeFiles, } from '../modules/torrent.js';
import { compressImage, uncompressedImage } from '../modules/media.js';
import { zipDirectory } from '../modules/zipper.js';
import { createProgress } from '../utils/progress.js';
import { tempFilePath, cleanupFile, formatSize, } from '../utils/tg.js';
import { TG_MAX_FILE_SIZE, isImageFile, isVideoFile, isTorrentFile, } from '../utils/constants.js';
/**
 * Handle a torrent file upload
 */
export async function handleTorrentFile(ctx) {
    const doc = ctx.message?.document;
    if (!doc)
        return;
    if (!isTorrentFile(doc.file_name || ''))
        return;
 ...
 */
export async function handleMagnetLink(ctx) {
...
async function downloadAndZip(ctx, info, source) {
    ...
    try {
        ...
        const { zipPath: finalZip, zipSize } = await zipDirectory(result.dirPath, zipPath, progress);
        await progress.update('📤 Sending ZIP file...');
        await ctx.replyWithDocument(new InputFile(finalZip, `${info.name}.zip`), {

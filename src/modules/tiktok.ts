 ...
 */
function extractSnaptikLink(html: string): string | null {
  ...
  for (const pat of patterns) {
    ...
    if (m) {
      ...
      if (link.startsWith('/')) link = 'https://snaptik.app' + link;
      return link;
    }
  }
  return null;
 ...
 */
function extractSsstikLink(html: string): string | null {
  ...
  for (const pat of patterns) {
    ...
    if (m) {
      let link = decodeHtmlEntities(m[1]);
      if (link.startsWith('//')) link = 'https:' + link;
      if (link.startsWith('/')) link = 'https://ssstik.io' + link;
      return link;
    }
  }
  return null;
...
async function tryPackage(
  ...
  try {
    ...
    const url =
      result?.videoUrl ?? result?.url ?? result?.downloadUrl ?? result?.video?.url;
...
export async function downloadTiktok(
  ...
  try {
    ...
    if (!downloadUrl) downloadUrl = await trySsstik(url, options);
    if (!downloadUrl) downloadUrl = await tryPackage(url, options);

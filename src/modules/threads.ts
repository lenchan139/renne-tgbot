 ...
 */
export async function downloadThreads(
  url: string,
  ...
  try {
    ...
    const jsonLdVideos = new Set<string>();
    ...
    for (const raw of jsonLdScripts) {
      try {
        ...
        for (const item of items) {
          ...
          if (item?.image) jsonLdImages.add(item.image);
          if (item?.video) jsonLdVideos.add(item.video);
        }

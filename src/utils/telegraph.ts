...
export function htmlToTelegraphNodes(html: string): TelegraphNode[] {
  ...
  const tagStack: { tag: string; attrs?: Record<string, string>; children: TelegraphNode[] }[] = [];
    ...
    if (h[i] === '<') {
      ...
      if (isClosing) {
        ...
        if (selfClosing) {
          ...
          // Add to parent
          const node: TelegraphNodeElement = { tag: tagName, attrs: Object.keys(attrs).length > 0 ? attrs : undefined };
          if (tagStack.length > 0) {
 ...
 */
export function wrapTelegraphHtml(
  title: string,
  paragraphs: string[],
